...
pylith::meshio::TestDataWriterPoints::_initialize(void) {
    ...
    const int meshDim = 0;
    delete _pointMesh;_pointMesh = new pylith::topology::Mesh(meshDim, PETSC_COMM_WORLD);assert(_pointMesh);

    PetscErrorCode err;
    PetscDM dmPoints = NULL;
    const PetscInt depth = 0;
    const size_t numPoints = data->numPoints;
    ...
    err = DMSetCoordinateDim(dmPoints, data->spaceDim);PYLITH_CHECK_ERROR(err);
    err = DMPlexCreateFromDAG(dmPoints, depth, dmNumPoints, &dmConeSizes[0], &dmCones[0],
                              &dmConeOrientations[0], data->points);PYLITH_CHECK_ERROR(err);
